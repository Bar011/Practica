...
using Practica.Server.Data.Models;
...
namespace Practica.Server.Controllers
{
    ...
    public class CiudadController : ControllerBase
    {
        ...
        public async Task<IActionResult> CreateCiudad([FromBody] Ciudad nuevaCiudad)
        {
            try
            {
                ...
                if (await _dataContext.Ciudades
                    .AsNoTracking()
                    .AnyAsync(ciudad => ciudad.Id_Ciudad == nuevaCiudad.Id_Ciudad))
                {
                    return BadRequest("Error: El id no es valido");
                }

                // Validar si no posee el mismo nombre que otra ciudad
                if (await _dataContext.Ciudades.AsNoTracking().AnyAsync(ciudad => ciudad.NombreCiudad.ToLower() == nuevaCiudad.NombreCiudad.ToLower()))
                {
        ...
        {
            return _dataContext.Ciudades.Any(c => c.Id_Ciudad == id);
        }
    }
}